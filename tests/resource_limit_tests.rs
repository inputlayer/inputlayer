...
//!
//! Tests for resource limit enforcement during query execution:
//! - Query timeout enforcement
//! - Memory tracking
//! - Result size limits
//! - Row width limits
//!
//! These tests verify the infrastructure in src/execution/limits.rs
//! and src/execution/timeout.rs works correctly in practice.

use inputlayer::{
    Config, StorageEngine,
    ResourceLimits, ResourceError, MemoryTracker,
    QueryTimeout, TimeoutError,
    ExecutionConfig,
};
use std::sync::Arc;
use std::thread;
...
fn test_intermediate_size_check() {
    ...
    if let Err(ResourceError::IntermediateResultExceeded { limit, actual, stage }) = result {
        assert_eq!(limit, 1000);
...
fn test_storage_engine_query_completes_quickly() {
    ...
    assert!(elapsed < Duration::from_secs(5), "Query took too long: {:?}", elapsed);
}
...
fn test_large_result_set_handling() {
    ...
    let results = storage.execute_query("result(X,Y) :- numbers(X,Y).").unwrap();
...
fn test_many_small_queries() {
    ...
    storage.insert("data", vec![(1, 2), (3, 4), (5, 6)]).unwrap();
