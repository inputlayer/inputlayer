//! Client Handler Unit Tests
//!
//! Tests for the Handler struct and its methods, covering:
...
fn test_get_storage_write() {
    ...
    // Insert some data using the storage API (use 2-column data for Tuple2 return type)
    let tuples = make_tuples_2col(&[(1, 10), (2, 20), (3, 30)]);
    let result = storage.insert_tuples("test", tuples);
    assert!(result.is_ok(), "Insert failed: {:?}", result.err());

    // Verify data was inserted (use rule-style query for Tuple2 result)
...
async fn test_query_count_increments() {
    ...
    let result = handler
        .query_program(None, "?- foo(X).".to_string())
        .await;
    ...
    let _ = handler
        .query_program(None, "?- bar(X).".to_string())
        .await;
...
fn test_validate_tuples_no_schema() {
    ...
    let result = handler.validate_tuples_against_schema("unregistered_relation", &tuples);
    assert!(result.is_ok());
}
...
fn test_validate_tuples_empty() {
    ...
    let result = handler.validate_tuples_against_schema("any_relation", &tuples);
    assert!(result.is_ok());
}
...
async fn test_query_program_simple() {
    ...
    {
        let mut storage = handler.get_storage_mut();
        storage.insert_tuples("numbers", make_tuples(&[1, 2, 3])).unwrap();
    }
...
async fn test_query_program_with_knowledge_graph() {
    ...
    // Create a new knowledge graph
    {
        ...
        storage.insert_tuples("kg_data", make_tuples(&[1, 2])).unwrap();
    }
...
async fn test_query_program_with_join() {
    ...
    {
        let mut storage = handler.get_storage_mut();
        storage.insert_tuples("edge", make_tuples_2col(&[(1, 2), (2, 3), (3, 4)])).unwrap();
        storage.insert_tuples("node", make_tuples(&[1, 2, 3])).unwrap();
    }
...
async fn test_concurrent_queries() {
    ...
    {
        let mut storage = handler.get_storage_mut();
        storage.insert_tuples("data", make_tuples(&[1, 2, 3, 4, 5])).unwrap();
    }
    ...
    assert_eq!(handler.total_queries(), 5, "Expected exactly 5 queries to be counted");
}
...
fn test_concurrent_read_write() {
    ...
    let writer_handle = thread::spawn(move || {
        for i in 2i64..5i64 {
            let mut storage = h.get_storage_mut();
...
async fn test_query_program_error_recovery() {
    ...
    let result2 = handler
        .query_program(None, "?- test(X).".to_string())
        .await;
...
async fn test_query_with_kg_switch() {
    ...
    // Create and populate two knowledge graphs
    {
        ...
        storage.insert_tuples("kg1_data", make_tuples(&[1])).unwrap();

        storage.create_knowledge_graph("kg2").unwrap();
        storage.use_knowledge_graph("kg2").unwrap();
        storage.insert_tuples("kg2_data", make_tuples(&[2])).unwrap();
    }
...
async fn test_query_result_schema() {
    ...
    assert_eq!(query_result.schema.len(), 2, "Schema should have exactly 2 fields");
}
...
fn test_handler_with_large_data() {
    ...
    // Insert large amount of data (2-column for Tuple2 return type)
    {
        let mut storage = handler.get_storage_mut();
    ...
    // Query should work (2-column query for Tuple2 result)
    {
        let mut storage = handler.get_storage_mut();
...
fn test_statistics_consistency() {
    ...
    // After some operations via storage (use 2-column for Tuple2)
    {
        let mut storage = handler.get_storage_mut();
