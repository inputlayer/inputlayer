...
//!
//! Tests for numeric edge cases that could cause panics or incorrect results:
//! - Division by zero handling
//! - Empty group aggregations
//! - Overflow/underflow behavior
//! - NaN/Infinity handling

use inputlayer::{Config, StorageEngine, Value, Tuple};
...
fn test_avg_with_single_value() {
    ...
    let _results = storage.execute_query("result(avg<V>) :- scores(_, V).").unwrap();
...
fn test_avg_multiple_values() {
    ...
    storage.insert("numbers", vec![(1, 10), (2, 20), (3, 30)]).unwrap();

    let _results = storage.execute_query("result(avg<V>) :- numbers(_, V).").unwrap();
...
fn test_count_single_value() {
    ...
    let _results = storage.execute_query("result(count<X>) :- items(X).").unwrap();
...
fn test_sum_multiple_values() {
    ...
    storage.insert("amounts", vec![(1, 10), (2, 20), (3, 30)]).unwrap();

    let _results = storage.execute_query("result(sum<V>) :- amounts(_, V).").unwrap();
...
fn test_min_max_single_value() {
    ...
    let _min_results = storage.execute_query("result(min<V>) :- vals(_, V).").unwrap();
    let _max_results = storage.execute_query("result(max<V>) :- vals(_, V).").unwrap();
...
fn test_arithmetic_division_normal() {
    ...
    let _results = storage.execute_query("result(X / Y) :- nums(X, Y).").unwrap();
    // Should not panic
}
...
fn test_int64_max_in_aggregation() {
    ...
    storage.insert("big_nums", vec![(1, i32::MAX), (2, 1)]).unwrap();
...
fn test_grouped_aggregation_one_group_empty() {
    ...
    let _results = storage.execute_query("result(G, avg<V>) :- sales(G, V).").unwrap();
...
fn test_multiple_aggregations_same_query() {
    ...
    storage.insert("data", vec![(1, 10), (2, 20), (3, 30)]).unwrap();

    // Multiple aggregations in one query
    let _results = storage.execute_query("result(count<X>, sum<V>, min<V>, max<V>) :- data(X, V).").unwrap();
    // Should not panic
}
