...
//!
//! These tests verify the system's resilience to:
//! - Partial/interrupted writes
//! - Corrupted files (WAL, metadata, Parquet)
//! - Truncated files
//! - Recovery from various crash scenarios
//!
//! CRITICAL: These tests are essential for production readiness.

use inputlayer::config::DurabilityMode;
use inputlayer::storage::persist::batch::Update;
use inputlayer::storage::persist::{consolidate, to_tuple2s, FilePersist, PersistBackend, PersistConfig};
...
fn create_test_persist(temp: &TempDir) -> FilePersist {
...
fn test_wal_double_replay_idempotency() {
    ...
    // Recover first time
    {
        ...
        let tuples = to_tuple2s(&consolidated);
...
fn test_recovery_with_corrupted_wal_json() {
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(err_msg.contains("WAL") || err_msg.contains("parse"),
                    "Error should mention WAL or parsing: {}", err_msg);
        }
    }
}
...
fn test_recovery_with_missing_required_metadata_fields() {
    ...
    assert!(result.is_err(), "Missing metadata fields should return error");
}
...
fn test_recovery_with_orphaned_wal_archives() {
    ...
    fs::write(path.join("wal/wal_12345.archived"), r#"{"shard":"old:data","update":{}}"#).unwrap();

    // System should start up without issues
    let persist = create_test_persist_with_config(path.clone(), 100);
    let updates = persist.read("db:edge", 0).unwrap();
    assert!(!updates.is_empty(), "Should recover valid data despite orphaned archives");
}
...
fn test_compaction_atomicity() {
    ...
    assert!(updates.iter().all(|u| u.time >= 10),
...
fn test_data_integrity_after_multiple_restarts() {
    ...
    for i in 0..3 {
        ...
        assert_eq!(tuples.len(), expected_data.len(),
                   "Restart {} should preserve data count", i);

        for (a, b) in &expected_data {
            assert!(tuples.contains(&(*a, *b)),
                    "Restart {} should preserve tuple ({}, {})", i, a, b);
        }
    }
}
...
fn test_compaction_preserves_all_data() {
    ...
    // Create data with many updates at different times
    {
        ...
        let tuples = to_tuple2s(&after);
        for i in 50..100 {
            assert!(tuples.contains(&(i, i * 2)),
                    "Tuple ({}, {}) should be preserved after compaction", i, i * 2);
        }
    }
    ...
    assert_eq!(updates.len(), 50, "Compacted data should persist across restart");
}
...
fn test_compaction_with_deletions() {
    ...
    assert!(tuples.contains(&(5, 6)), "(5,6) should be preserved");
}
...
fn test_many_restarts_with_incremental_data() {
    ...
    for cycle in 0..10 {
        ...
        let updates = persist.read("db:edge", 0).unwrap();
        assert_eq!(updates.len(), (cycle + 1) as usize,
                   "Cycle {} should have {} updates", cycle, cycle + 1);
    }
}
