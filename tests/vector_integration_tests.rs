...
fn test_euclidean_distance_basic() {
    ...
    storage
        .insert_tuples(
            "embedding",
            vec![id_vec_tuple(1, vec![3.0, 4.0])],
        )
        .unwrap();

    // Euclidean distance from origin should be 5.0 (3-4-5 triangle)
    let results = storage
        .execute_query_with_rules_tuples("result(Id, D) :- embedding(Id, V), D = euclidean(V, [0.0, 0.0]).")
        .unwrap();

    assert!(!results.is_empty(), "Should have results for euclidean distance");
}
...
fn test_cosine_similarity_basic() {
    ...
    storage
        .insert_tuples(
            "embedding",
            vec![id_vec_tuple(1, vec![1.0, 0.0, 0.0])],
        )
        .unwrap();

    let results = storage
        .execute_query_with_rules_tuples("result(Id, D) :- embedding(Id, V), D = cosine(V, [0.0, 1.0, 0.0]).")
        .unwrap();

    assert!(!results.is_empty(), "Should have results for cosine similarity");
}
...
fn test_dot_product_basic() {
    ...
    storage
        .insert_tuples(
            "embedding",
            vec![id_vec_tuple(1, vec![1.0, 2.0, 3.0])],
        )
        .unwrap();

    let results = storage
        .execute_query_with_rules_tuples("result(Id, D) :- embedding(Id, V), D = dot(V, [4.0, 5.0, 6.0]).")
        .unwrap();
...
fn test_manhattan_distance_basic() {
    ...
    let results = storage
        .execute_query_with_rules_tuples("result(Id, D) :- embedding(Id, V), D = manhattan(V, [0.0, 0.0, 0.0]).")
        .unwrap();

    assert!(!results.is_empty(), "Should have results for manhattan distance");
}
...
fn test_pairwise_cosine_similarity() {
    ...
    assert_eq!(results.len(), 3, "Pairwise cosine should have 3 pairs, got {}: {:?}", results.len(), results);
}
...
fn test_vector_single_element() {
    ...
    let results = storage
        .execute_query_with_rules_tuples("result(Id, D) :- embedding(Id, V), D = euclidean(V, [0.0, 0.0, 0.0]).")
        .unwrap();

    assert_eq!(results.len(), 1, "Single element query should have 1 result");
    ...
    assert!(pairwise.is_empty(), "Single element pairwise should have no pairs");
}
...
fn test_vector_self_comparison() {
    ...
    assert_eq!(results.len(), 1, "Self comparison should have 1 result (1x1 Cartesian)");
}
...
fn test_vector_inline_rule() {
    ...
    assert_eq!(results.len(), 3, "Inline distance query should produce 3 results");
}
...
fn test_vector_inline_similarity() {
    ...
    // 4 choose 2 = 6 pairs
    assert_eq!(results.len(), 6, "Should have 6 pairwise similarity results");
}
...
fn test_all_vector_operations() {
    ...
    let euclidean = storage
        .execute_query_with_rules_tuples("result(Id, D) :- v(Id, V), D = euclidean(V, [0.0, 0.0, 0.0]).")
        .unwrap();
    assert_eq!(euclidean.len(), 2, "Euclidean should work");

    let cosine = storage
        .execute_query_with_rules_tuples("result(Id, D) :- v(Id, V), D = cosine(V, [1.0, 0.0, 0.0]).")
        .unwrap();
    ...
    let manhattan = storage
        .execute_query_with_rules_tuples("result(Id, D) :- v(Id, V), D = manhattan(V, [0.0, 0.0, 0.0]).")
        .unwrap();
...
fn test_cartesian_full() {
    ...
    assert_eq!(results.len(), 4, "Full Cartesian product should have 4 results");
}
...
fn test_pairwise_vector_regression() {
    ...
    // Should have 3 pairs
    assert_eq!(results.len(), 3, "Pairwise regression: expected 3 pairs, got {}", results.len());
}
