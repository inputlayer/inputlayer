...
//! These tests use tower's test utilities to test the API without starting a server.
...
async fn test_create_and_get_knowledge_graph() {
    ...
    // Get the created KG
    let (status, json) = send_json_request(&app, "GET", "/api/v1/knowledge-graphs/test_kg", None).await;
...
async fn test_delete_knowledge_graph() {
    ...
    // Delete it
    let (status, json) = send_json_request(&app, "DELETE", "/api/v1/knowledge-graphs/to_delete", None).await;

    assert_eq!(status, StatusCode::OK);
    assert!(json["success"].as_bool().unwrap_or(false));

    // Verify it's deleted
    let (status, _json) = send_json_request(&app, "GET", "/api/v1/knowledge-graphs/to_delete", None).await;
...
async fn test_get_nonexistent_knowledge_graph() {
    ...
    let (status, _json) = send_json_request(&app, "GET", "/api/v1/knowledge-graphs/nonexistent", None).await;
