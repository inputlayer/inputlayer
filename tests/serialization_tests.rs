...
//! - Value types (Int32, Int64, Float64, String, Bool, Null, Vector, VectorInt8, Timestamp)
//! - REST API DTOs
//! - Edge cases (NaN, Infinity, Unicode, empty collections)
//!
//! These tests ensure data integrity across serialization boundaries.

use inputlayer::value::{Value, Tuple, DataType};
...
fn test_int32_json_roundtrip() {
    ...
    for original in values {
        ...
        assert_eq!(original, deserialized, "Int32 roundtrip failed for {:?}", original);
    }
}
...
fn test_int64_json_roundtrip() {
    ...
    for original in values {
        let json = serde_json::to_string(&original).expect("Serialization failed");
        let deserialized: Value = serde_json::from_str(&json).expect("Deserialization failed");
        assert_eq!(original, deserialized, "Int64 roundtrip failed for {:?}", original);
    }
}
...
fn test_string_json_roundtrip() {
    ...
    for original in values {
        let json = serde_json::to_string(&original).expect("Serialization failed");
        let deserialized: Value = serde_json::from_str(&json).expect("Deserialization failed");
        assert_eq!(original, deserialized, "String roundtrip failed for {:?}", original);
    }
}
...
fn test_bool_json_roundtrip() {
    ...
    for original in values {
        let json = serde_json::to_string(&original).expect("Serialization failed");
        let deserialized: Value = serde_json::from_str(&json).expect("Deserialization failed");
        assert_eq!(original, deserialized, "Bool roundtrip failed for {:?}", original);
    }
}
...
fn test_timestamp_json_roundtrip() {
    ...
    for original in values {
        let json = serde_json::to_string(&original).expect("Serialization failed");
        let deserialized: Value = serde_json::from_str(&json).expect("Deserialization failed");
        assert_eq!(original, deserialized, "Timestamp roundtrip failed for {:?}", original);
    }
}
...
fn test_special_float_values_serialization() {
    ...
    assert!(inf_json.contains("null"), "Infinity should serialize value as null: {}", inf_json);
    assert!(neg_inf_json.contains("null"), "Neg infinity should serialize value as null: {}", neg_inf_json);
    assert!(nan_json.contains("null"), "NaN should serialize value as null: {}", nan_json);
}
...
fn test_unicode_string_serialization() {
    ...
    for original in values {
        let json = serde_json::to_string(&original).expect("Serialization failed");
        let deserialized: Value = serde_json::from_str(&json).expect("Deserialization failed");
        assert_eq!(original, deserialized, "Unicode roundtrip failed for {:?}", original);
    }
}
