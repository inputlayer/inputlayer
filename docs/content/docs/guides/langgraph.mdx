...
This guide walks through the integration from install to resumable agents with semantic memory. No prior Datalog or IQL experience required.
...
Long-running agents accumulate checkpoints. Two methods help you manage growth:

```python
# Keep only the 10 most recent checkpoints for a thread
await checkpointer.prune_thread("user-42", keep_last=10)

# Wipe a thread entirely (checkpoints + intermediate writes)
...
Both have sync variants (`prune_thread_sync`, `delete_thread`) for non-async contexts.
...
From `SqliteSaver` or `PostgresSaver`: same swap, but also run the one-time data copy below if you want history preserved. The schema is relation-based, so you can replay old checkpoints by inserting into `graph_checkpoint` and `graph_write` directly. Most teams start fresh: point the new checkpointer at a clean KG and let it accumulate new history as agents run. Old checkpoints stay in the old store until you drop it.
...
The built-in topic extractor is keyword-based and meant for demos. In production, extract topics with an LLM and pass them explicitly to `astore()`. The ontology is what matters, not the extractor.
...
The SDK ships with 11 runnable examples at [`examples/langgraph/`](https://github.com/inputlayer/inputlayer/tree/main/packages/inputlayer-py/examples/langgraph):
...
**`ValueError: thread_id contains characters the IQL parser cannot safely embed`.** Thread identifiers must avoid `,`, `)`, `<`, and `>` and any ASCII control byte (other than tab, newline, carriage return). These characters confuse the current IQL parser when a string literal appears inside a multi-argument call. Use alphanumeric plus `._-:/@+=` for portable IDs. Content and role are base64-encoded internally, so they are not affected by this limitation.
