...
This pulls in `langgraph` and `langchain-core` alongside the InputLayer SDK. You'll also need a running InputLayer server.
...
`kg_router` is a conditional-edge function. Each branch maps a target node to an IQL query. The first branch whose query returns a non-empty result wins. If no branch matches, the router returns the `default`:
