...
The `il` CLI ships with the `inputlayer` crate:
...
playground     consistency-core         1.0.2     sha256:947df8d140d3650...
```
...
Because extraction is ontology-bound and quote-gated, a conversation that tries to inject facts gets nowhere. Embedded "system override" instructions demanding fabricated claims produce no findings; a claim whose quote does not appear verbatim in the cited message is dropped and reported in `dropped`; and mapped values are typed at the template layer, so extracted text can never escape into the fact language. The failure direction is always the safe one: a missed finding, never a forged fact or a false conflict.
