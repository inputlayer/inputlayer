...
**Returns:** `t2 - t1` in milliseconds (can be negative).
...
% Score items by recency-weighted interactions
+item_score(UserId, ItemId, sum<Score>) :-
    interactions(UserId, ItemId, Action, Ts),
    Now = time_now(),
    BaseScore = case(Action, "purchase", 10.0, "view", 1.0),
...
| `time_diff` | `time_diff(t1, t2)` | t2 - t1 (ms) |
...
- [Aggregations Reference](../reference/aggregations.md) - TopK for time-based ranking
- [Vector Search](vectors.md) - Combine temporal decay with similarity
- [Configuration](configuration.md) - Persistence settings for event data
