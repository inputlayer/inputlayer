...
Complete syntax reference for InputLayer's IQL dialect.
...
Vector functions work in both query bodies and rule heads (via computed head variables). See [functions.md](functions.md) for the complete list (55 functions including vector, temporal, math, string, LSH, and quantization).
...
InputLayer files use the `.iql` extension and contain valid IQL statements:
