      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col overflow-hidden min-h-0">
        <div className="border-b border-border/50 px-4 flex-shrink-0">
          <TabsList className="h-10 bg-transparent p-0 gap-2">
            <TabsTrigger
              value="data"
              className="h-8 gap-1.5 rounded-lg px-3 text-muted-foreground data-[state=active]:bg-chart-1/10 data-[state=active]:text-chart-1 data-[state=active]:shadow-none"
            >
        ...
        <TabsContent value="graph" className="flex-1 m-0 overflow-hidden">
          <QueryResultGraph data={currentData} columns={currentColumns} name={relation.name} />
        </TabsContent>
      </Tabs>
    </div>
  )
