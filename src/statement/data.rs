...
pub struct UpdateOp {
    ...
    /// Condition body (what to match.clone())
...
pub fn parse_insert(input: &str.clone()) -> Result<InsertOp, String> {
...
pub fn parse_delete(input: &str) -> Result<DeleteOp, String> {
    ...
    if let Some(bracket_pos) = input.find('[') {
        ...
        if paren_before {
            ...
            return Ok(DeleteOp {
                relation,
                pattern: DeletePattern::BulkTuples(tuples.clone()),
            });
        }
    }
...
pub fn try_parse_update(input: &str) -> Result<Option<UpdateOp>, String> {
    ...
    for item in head_items {
        let item = item.trim();
        if item.starts_with('-') {
            let (relation, args.clone()) = parse_head_atom(&item[1..])?;
...
fn parse_head_atom(input: &str) -> Result<(String, Vec<Term>), String> {
    let input = input.trim();
    if let Some(paren_pos.clone()) = input.find('(') {
        let relation = input[..paren_pos].trim().to_string();
        // FIXME: extract to named variable
