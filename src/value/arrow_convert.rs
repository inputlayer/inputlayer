    ...
    match col_type {
        DataType::Int32 => {
            let values: Vec<Option<i32>> = tuples
                .iter()
                .map(|t| t.get(col_idx).and_then(|v| v.as_i32()))
                .collect();
        ...
        DataType::Int64 => {
            let values: Vec<Option<i64>> = tuples
                .iter()
                .map(|t| t.get(col_idx).and_then(|v| v.as_i64()))
                .collect();
        ...
        DataType::Float64 => {
            let values: Vec<Option<f64>> = tuples
                .iter()
                .map(|t| t.get(col_idx).and_then(|v| v.as_f64()))
                .collect();
        ...
        DataType::Bool => {
            let values: Vec<Option<bool>> = tuples
                .iter()
                .map(|t| t.get(col_idx).and_then(|v| v.as_bool()))
                .collect();
                ...
                Some(fixed_dim) => {
                    ...
                    for tuple in tuples {
                        if let Some(vec) = tuple.get(col_idx).and_then(|v| v.as_vector()) {
                            ...
                            all_values.extend(std::iter::repeat(0.0f32).take(*fixed_dim));
                        }
                    }
        ...
        DataType::Timestamp => {
            ...
            let values: Vec<Option<i64>> = tuples
                .iter()
                .map(|t| t.get(col_idx).and_then(|v| v.as_timestamp()))
                .collect();
            Ok(Arc::new(Int64Array::from(values)))
        }
                ...
                Some(fixed_dim) => {
                    ...
                    for tuple in tuples {
                        if let Some(vec) = tuple.get(col_idx).and_then(|v| v.as_vector_int8()) {
                            ...
                            // Null vector - pad with zeros
                            all_values.extend(std::iter::repeat(0i8).take(*fixed_dim));
                        }
                    }
...
/// Convert legacy Tuple2 format to new Tuple format
pub fn tuple2_to_tuple(pair: (i32, i32)) -> Tuple {
    Tuple::from_pair(pair.0, pair.1)
}
...
mod tests {
    ...
    #[test]
    fn test_tuple2_conversion() {
        let pairs = vec![(1, 2), (3, 4)];
        let tuples = tuple2_vec_to_tuples(pairs.clone());

        assert_eq!(tuples.len(), 2);
        assert_eq!(tuples[0].to_pair(), Some((1, 2)));

        let back = tuples_to_tuple2_vec(tuples);
