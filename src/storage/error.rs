...
pub enum StorageError {
    ...
    /// Differential Dataflow computation error
    ///
    /// Occurs when DD worker fails to process an operation (insert, delete, query).
    /// This is a critical error - the in-memory and DD state may be inconsistent.
    #[error("DD computation error: {0}")]
    DDComputationError(String),
